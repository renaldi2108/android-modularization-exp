    ...
    Scaffold(modifier = modifier, snackbarHost = { SnackbarHost(snackbarHostState) }) { padding ->
        Column(modifier = Modifier.fillMaxSize().padding(padding)) {
            AppTextField(
                ...
                keyboardActions = KeyboardActions(onSearch = { onAction(RecipeListAction.Submit) }),
                ...
                state.errorMessage != null && state.recipes.isEmpty() ->
                    Box(Modifier.fillMaxSize(), Alignment.Center) {
                        Column(horizontalAlignment = Alignment.CenterHorizontally) {
                            ...
                            AppButton("Coba lagi", { onAction(RecipeListAction.Retry) }, Modifier.padding(top = AppTheme.dimens.space))
                        }
                    }
                    ...
                    items(state.recipes, key = { it.id }) { recipe -> RecipeRow(recipe) { onRecipeClick(recipe.id) } }
                }
            }
        }
    }
...
private fun RecipeRow(recipe: Recipe, onClick: () -> Unit) {
    Card(onClick = onClick, modifier = Modifier.fillMaxWidth()) {
        Column(
            ...
            Text(recipe.name, style = AppTheme.typography.titleMedium, maxLines = 1, overflow = TextOverflow.Ellipsis)
            Text("${recipe.cuisine} · ${recipe.difficulty}", style = AppTheme.typography.bodySmall, color = AppTheme.colors.onSurfaceVariant)
            Text("★ ${recipe.rating}  ·  ${recipe.cookTimeMinutes} mnt", style = AppTheme.typography.labelMedium, color = AppTheme.colors.primary)
        }
    }
}
